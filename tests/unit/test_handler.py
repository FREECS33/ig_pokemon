...
def apigw_update_event():
    return {
        ...
        "pathParameters": {"proxy": "/update_pokemon"},
        "httpMethod": "POST",
        "stageVariables": {"baz": "qux"},
        "path": "/update_pokemon",
    }
...
@pytest.fixture()
def apigw_add_interaction_event():
    return {
        "body": json.dumps({
            "Fk_id_user": 1,
            "Fk_id_pokemon": 1,
        ...
        "pathParameters": {"proxy": "/add_interaction"},
        "httpMethod": "POST",
        "stageVariables": {"baz": "qux"},
        "path": "/add_interaction",
    }
...
def test_update_pokemon_lambda(apigw_update_event):
    ...
    if ret["statusCode"] == 200:
        data = json.loads(ret["body"])
        print("Response:", data)
        assert data["message"] == "Pokemon updated successfully"
    else:
        print("Error:", ret["body"])


def test_add_interaction_lambda(apigw_add_interaction_event):
    ret = add_interaction_app.lambda_handler(apigw_add_interaction_event, "")
